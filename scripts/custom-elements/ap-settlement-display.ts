...
import { Settlement } from "../generators/settlement/settlements.js";
import { AuthContainerElement } from "./ap-auth-container.js";
import { PopulationTableElement } from "./ap-population-table.js";
...
export class SettlementDisplayElement extends HTMLElement {
	...
	BuildTownInformation(): HTMLTableElement {
		...
		table.appendChild(
			this.BuildTableRow("Gold Limit:", `${this.settlement.goldLimit.toLocaleString()} gp (${onHand.toLocaleString()} gp on hand)`)
		);
	...
	BuildPopulationBreakdown(): AuthContainerElement {
		const container = document.createElement("ap-auth-container") as AuthContainerElement;
		...
		const popTable = document.createElement("ap-population-table") as PopulationTableElement;
	...
	BuildTableRow(header: string, data: string): HTMLTableRowElement {
